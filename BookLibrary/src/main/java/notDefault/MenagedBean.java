...
import notDefault.DataBase.Author;
import notDefault.DataBase.Book;
import notDefault.DataBase.Order;
import notDefault.DataBase.Reader;
...
public class MenagedBean {
    ...
    public List<Order> getOrder() {
        ...
        if (chosenOrder.getOrder_date() != null) {
            predicates.add(cb.equal(root.get("order_date").as(Date.class), chosenOrder.getOrder_date()));
        }
        if (chosenOrder.getReturn_date() != null) {
            predicates.add(cb.equal(root.get("return_date"), chosenOrder.getReturn_date()));
        }
    ...
    public String addOrder(String bookName, String readerName, Date orderDate, Date returnDate) {
        Book book = findBook(bookName);
        Reader reader = findReader(readerName);

        Order order = new Order();
        order.setBook_id(book);
        order.setReader_id(reader);
    ...
    public String updateOrder(int id, String bookName, String readerName, Date orderDate, Date returnDate) {
        try {
            ...
            em.getTransaction().begin();
            foundOrder.setBook_id(book);
            foundOrder.setReader_id(reader);
            foundOrder.setOrder_date(orderDate);
            foundOrder.setReturn_date(returnDate);
            em.getTransaction().commit();
